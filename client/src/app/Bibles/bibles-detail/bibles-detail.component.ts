...
export class BiblesDetailComponent implements OnInit {
  ...
  constructor(
    private tbibleService: TBibleService,
  ...
  ngOnInit(): void {
    this.loadBible();
    this.tbibleService.getMax().subscribe((response) => {
  ...
  loadBible() {
    this.tbibleService
      .getBible1(this.route.snapshot.paramMap.get('id'))
      .subscribe((tbible) => {
        this.tbible = tbible;
      });
  }
  ...
  loadBibles() {
    this.tbibleService.getBible(this.findForm.value).subscribe((response) => {
      ...
      this.router
        ...
        .then(() => this.router.navigate(['/bibles/' + this.selectedIndex]));
    });
  }
  ...
  updateBible() {
    this.tbibleService
      .updateMember(this.tbible, this.route.snapshot.paramMap.get('id'))
      .subscribe(() => {
  ...
  previous() {
    this.route.paramMap.subscribe((params) => {
      if (
        ...
        localStorage.getItem('cSearch') == null
      ) {
        var id = params.get('id');
        this.selectedIndex = Number(id);
        --this.selectedIndex;
